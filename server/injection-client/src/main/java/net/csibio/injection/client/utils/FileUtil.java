...
public class FileUtil {
     ...
     * @return 找到的文件列表,key为platform, value为该platform文件夹下的所有.json格式的文件位置
     */
    public static HashMap<String, List<File>> scanProjectFiles(String projectName) {
        // 提取目录名称
        ...
        if (platformFolders != null) {
            for (File platformFolder : platformFolders) {
                if (platformFolder.isDirectory()) {
                    ...
                    File[] filesInPlatform = platformDir.listFiles();
                    List<File> fileList = new ArrayList<>();
                    if (filesInPlatform != null) {
                        for (File file : filesInPlatform) {
                            if (file.isFile() && file.getName().toLowerCase().endsWith(SuffixConst.JSON)) {
                                fileList.add(file);
                            }
                        }
                    }
                    fileMap.put(platformFolder.getName(), fileList);
                }
            }
        }
    ...
    public static List<File> scanPlatformFiles(String projectName, String platform) {
        String platformPath = RepositoryUtil.getPlatformName(projectName, platform);
        File platformDir = new File(platformPath);
        File[] filesInPath = platformDir.listFiles();
