...
import net.csibio.injection.client.domain.vo.SampleLocation.*;
import net.csibio.injection.client.domain.vo.order.*;
import net.csibio.injection.client.domain.vo.sample.SampleExcelErrorVO;
import net.csibio.injection.client.domain.vo.sample.SamplePositionVO;
import net.csibio.injection.client.service.*;
import net.csibio.injection.core.excel.PreOrderExcelManager;
import net.csibio.injection.core.excel.SampleExcelManager;
import net.csibio.injection.core.utils.DateUtil;
import net.csibio.injection.core.validate.PreOrderValidate;
...
import static net.csibio.injection.client.constants.Constants.SINGLE_BOARD_SAMPLE_SIZE;
...
public class PreOrderController extends BaseController<PreOrderDO, PreOrderQuery> {
    ...
    Result removeSample(PreOrderRemoveSampleVO removeSampleVO) {
        ...
        checkIsNotNull(preOrderDO, ResultCode.PRE_ORDER_IS_EMPTY);
    ...
    Result deleteSample(@RequestParam(value = "preOrderId", required = true) String orderId,
        ...
        checkIsNotNull(sample, ResultCode.SAMPLE_NOT_EXISTED);
        // 删除前处理工单中的记录
    ...
    Result blurList(PreOrderSampleQuery query) {
        checkIsNotNull(query.getPreOrderId(), ResultCode.PRE_ORDER_ID_CANNOT_BE_EMPTY);
        checkIsNotNull(query.getSampleNo(), ResultCode.SAMPLE_NO_CANNOT_BE_EMPTY);
        checkIsNotNull(query.getProjectId(), ResultCode.PROJECT_ID_CANNOT_BE_NULL);
    ...
    Result addSample(@RequestBody PreOrderSampleAddVO preOrderSampleAddVO) {
        checkIsNotNull(preOrderSampleAddVO.getPreOrderId(), ResultCode.PRE_ORDER_ID_CANNOT_BE_EMPTY);
        checkIsNotNull(preOrderSampleAddVO.getSampleNo(), ResultCode.SAMPLE_NO_CANNOT_BE_EMPTY);
        checkIsNotNull(preOrderSampleAddVO.getProjectId(), ResultCode.PROJECT_ID_CANNOT_BE_NULL);
        SampleDO sampleDO = sampleService.getBySampleNo(preOrderSampleAddVO.getSampleNo(),
                preOrderSampleAddVO.getProjectId());
        checkIsNotNull(sampleDO, ResultCode.SAMPLE_NOT_EXISTED);
    ...
    void export(HttpServletResponse response,
        ...
        checkIsNotNull(orderId, ResultCode.PRE_ORDER_ID_CANNOT_BE_EMPTY);
    ...
    Result getProjectBoardList(
        ...
        checkIsNotNull(projectIds, ResultCode.PROJECT_ID_CANNOT_BE_EMPTY);
    ...
    Result update(PreOrderUpdateVO preOrderUpdateVO) {
        checkIsNotNull(preOrderUpdateVO.getId(), ResultCode.PRE_ORDER_ID_CANNOT_BE_EMPTY);
    ...
    Result orderCopy(PreOrderCopyVO preOrderCopyVO) {
        checkIsNotNull(preOrderCopyVO.getOwner(), ResultCode.PRE_ORDER_OWNER_CANNOT_BE_EMPTY);
        checkIsNotNull(preOrderCopyVO.getType(), ResultCode.PRE_ORDER_TYPE_CANNOT_BE_EMPTY);
        checkIsNotNull(preOrderCopyVO.getOrderId(), ResultCode.PRE_ORDER_ID_CANNOT_BE_EMPTY);
    ...
    private PreOrderDO buildWhiteListOrderAddDO(WhiteListPreOrderVO orderAdd) {
        PreOrderDO preOrderDO = new PreOrderDO();
        ProjectDO projectDO = projectService.getById(orderAdd.getProjectId());
        preOrderDO.setName(String.format("%s_%s", projectDO.getAlias(),
                parseDateToStr(new Date(), DateUtil.DATE_TIME_FORMAT_YYYYMMDDHHMISS)));
        ...
        preOrderDO.setSaveType(orderAdd.getSaveType());
        preOrderDO.setStatus(PreOrderStatus.VALID.getCode());
        preOrderDO.setType(PreOrderType.WHITELIST.getCode());
        return preOrderDO;
    }
